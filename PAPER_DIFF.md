...
문턱 0.5에 못 미쳐 **8개 개입 전부에 무반응**이다. 두 기업이 철강 누적 gap의 **44.6%**
(340.9 / 763.8 Mt; 전체 794.4 Mt 기준 42.9%)를 차지하는데 개입 엔진이 할 말이 없다.
〔정정 2026-08-04: 최초 기재한 "31% (340.9/958.6)"은 **B3 수정 *전* 분모를 수정 *후* 분자에
붙인 산술 오류**였다. 수정 전 총계는 1,096.8 Mt(345.5/1096.8 = 31.5%), 수정 후는 794.4 Mt.
958.6은 어느 쪽도 아니다. 사각지대의 크기는 알려진 것보다 컸다.〕 "사적 전환이 없다"는 발견 자체는 맞지만,
...
## 갱신 15 (2026-08-04 — 격차의 정체: 가격 불일치가 섞여 있다)

저자 지적(2026-08-04)에서 출발한 검증. **헤드라인 해석이 바뀐다.**

### 발견 1 — 불확실성의 부호는 수소가격에 조건부다 ($3/kg 경계)

포항 BF4(A06, T_required 2034)에서 σ 스케일과 p_h2를 동시에 움직인 실측:

| p_h2 | τ*(σ×1.0) | τ*(σ×0.1) | 불확실성의 역할 |
|---|---|---|---|
| **$5.5 (현행 config)** | 2050 | **없음** | **방아쇠** — 제거하면 전환 소멸 |
| $4.0 | 2049 | **없음** | 방아쇠 |
| $3.0 | 2047 | 2051 | 경계 |
| $2.0 | 2046 | **2045** | 정상 (교과서 WEDGE) |
| $1.5 | 2044 | **2042** | 정상 |

현행 가격에서 h2-DRI의 결정론적 연간 마진은 t당 **−333달러**이고, 학습곡선
(수소 −5%/yr·전력 −1.5%·CAPEX −3%)이 2050년쯤 −15달러 근처까지 올리지만 여전히 음수다.
**기댓값으로는 전환하지 않고, 유리한 꼬리(싼 수소·비싼 탄소)만이 문턱을 넘는다** (p_ex 0.55).
그래서 σ를 자르면 방아쇠가 사라진다 — h2_cfd의 Δτ* +1.11y가 여기서 완전히 설명된다.
버그도 측도 문제도 아니었다 (갱신 12~14의 잠정 판독 종결).

**단, 이는 구조적 성질이 아니라 p_h2 = $5.5에 조건부다.** FORMULA_LEDGER §E의
"「불확실성이 자본을 막는다」는 현행 엔진의 산출이 아니다"는 **가격 조건부**로 정정한다.

### 발견 2 — 엔진이 두 개의 시나리오를 동시에 돌린다 (구조적)

| 무엇 | 출처 | 세계 |
|---|---|---|
| T_required (요구 전환 시점) | IAM 배치 경로 | **탄소중립 시나리오** |
| p_h2 $5.5 (μ −5%/yr) | IMARC H2 Index, DOE LCOH | **현재 시장 + 학습 가정** |
| p_elec $75/95 (μ −1.5%/yr) | KEPCO 요금 | 현재 시장 |

IAM에서 DRI-EAF-H₂가 배치되는 **이유가 그 시나리오의 내생 수소·전력 가격**이다.
수량만 가져오고 가격은 시장에서 가져오면, 우리가 재는 격차에 **가격 불일치**가 섞인다.
즉 현행 격차 = ① 가격 격차(IAM 전제 가격 vs 시장 제공 가격) + ② 유인 격차(같은 가격에서의
사적-사회적 차이) 이고, **어느 쪽이 지배적인지 모른다.** 발견 1의 민감도($5.5→$2.0에서
τ* −4년, 불확실성 부호 반전)로 보아 ①이 클 가능성이 있다.

### 판정 — 약점이 아니라 분해해야 할 대상

①이 크다면 그것 자체가 정책적으로 더 무거운 발견이다: **요구 경로는 시장이 제공하지 않는
수소가격을 전제한다.** 수소 공급 정책이 철강 전환의 선행조건이라는 뜻이고, CfD의 역할이
리스크 절감이 아니라 **수준 인하**라는 결론(갱신 14 §D의 tenor·헤지가능성 결과)과 정합한다.

**필요한 실험**: IAM 자신의 가격에서 사적 최적을 다시 푼다. 남는 격차가 순수한 ②다.
선행 조건은 X10 데이터 — 요청 명세 3번에 **수소·전력 가격 경로**를 추가했다
(`data/raw/gcam/MISSING.md`; 이전 명세에는 빠져 있었다).

### 부수 — 강제 셧다운 브랜치 부재 확인

자산의 상태는 「현행 강도」와 「전환 후 잔여 강도」 둘뿐이다. **용량이 0이 되는 경로가 없다** —
폐쇄·조기퇴역·생산 이전 미구현. 그 직접적 증상: `no_feasible_route`인 HYUNDAI의 요구 경로가
BAU로 폴백되어 **누적 격차 0.0 MtCO₂** — 전환할 수 없다고 명시한 유일한 기업이 완벽 정렬로
채점된다. GCAM-ROK 강화 시나리오의 핵심 레버("2040년 이후 고로 수명연장 금지")를 현행
엔진은 표현할 수 없다. reline 결정의 선택지는 원래 셋(재축·전환·폐쇄)인데 둘만 모델링한다.
