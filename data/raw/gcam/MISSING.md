...
3. **같은 시나리오의 수소·전력 가격 경로** (연도 × USD/kg-H₂, USD/MWh, 실질 기준연도).
   **가장 결정적인 요청 항목이다.** 현행 CAP은 배치 **수량**은 IAM에서, **가격**은 시장
   (IMARC H2 Index $5.5/kg, KEPCO 요금)에서 가져와 **두 시나리오를 동시에 돌린다**.
   그런데 IAM에서 DRI-EAF-H₂가 배치되는 이유가 그 시나리오의 내생 수소가격이므로,
   수량만 가져오면 우리가 재는 격차에 **가격 불일치**가 섞인다. 실측: p_h2를 5.5 → 2.0
   으로 내리면 포항 4고로 τ*가 2050 → 2046으로 당겨지고, **불확실성의 부호까지 뒤집힌다**
   ($3/kg 경계 — 그 위에서는 변동성이 전환의 방아쇠, 아래에서는 지연 요인).
   IAM 가격에서 사적 최적을 다시 풀어야 순수한 사적-사회적 유인 격차가 분리된다.
4. 시나리오 정의 (CCS 가용성·수소 가용성 제약의 구체적 수치)와 GCAM 버전.
5. 가능하면 iron & steel 기술별(BF, BF-CCS, DRI-EAF, DRI-EAF-H₂, EAF-scrap) 산출 분해.
