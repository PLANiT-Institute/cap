...
def band_draw(cal: CalibrationSet, rng: np.random.Generator, elec_driver: str):
    ...
    sig = np.array([draw_sigma("carbon_diffusion"), draw_sigma("h2"), draw_sigma(elec_driver), draw_sigma("capex")])
...
def main() -> int:
    ...
    write_artifact(
        ...
        cal.param_status,
        uses=[],
        note="Fig 5 — σ·ρ band draw 포락. 노출 E는 base 고정 (공분산 불확실성만)",
    )
    ...
    write_artifact(
        ...
        cal.param_status,
        uses=["k", "ev_usd_bn"],  # 수준 필드는 conditional; 불변성 자체는 항등
        note="Fig 8 / Prop 1 — λ×p_bind 격자: share 벡터 불변, 수준만 스윙 (#claim-lambda-invariance)",
    )

    # 클러스터 분리 (band draw 기반)
    h2_carbon = np.concatenate([np.array(draws[f])[:, 0] for f, m in metas.items() if m["route"] == "h2_dri"])
    grid_carbon = np.concatenate([np.array(draws[f])[:, 0] for f, m in metas.items() if m["route"] != "h2_dri"])
    write_artifact(
        "cluster_separation",
        {
        ...
        cal.param_status,
        uses=[],
        note="두 클러스터 불교차 — 절반은 발견, 절반은 A4 배정의 귀결 (은폐하지 않음)",
    )
    ...
    write_artifact(
        ...
        cal.param_status,
        uses=[],
        note="R1 대응 — A5(λ 균일성) 스트레스: s_k = λ_k·RC_k/Σλ_j·RC_j",
    )
    print(
