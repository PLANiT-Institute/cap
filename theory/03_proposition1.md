...
**수치 데모**: λ×p_bind 격자 전체에서 share 벡터의 최대 이탈은 {{invariance.max_share_deviation}} (소수점 {{invariance.share_decimals}}자리 불변), 수준은 {{invariance.level_min_bps}}–{{invariance.level_max_bps}}bps로 스윙 (`model/s05_robustness.py` → `outputs/lambda_invariance.json`).
...
**λ_k 스트레스 결과**: driver별 λ_k 허용 시 기업별 share 최대 이동 {{lambda_k.max_shift_pct}} (`outputs/lambda_k_sensitivity.json`).
