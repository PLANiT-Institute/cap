...
현행 발견: H₂ CfD 단독(계약가 ${{iv.h2_cfd_price}}/kg, coverage·tenor 반영)은 τ*를 앞당기지 못하고 **오히려 늦춘다** (POSCO {{iv.POSCO.h2_cfd_dtau}}y, charge {{iv.POSCO.h2_cfd_dcharge}}bps). 두 메커니즘이 겹친다: ① blended 수소가로도 route가 여전히 사적 적자인데 σ-절단이 행사 트리거인 유리한 실현(싼 수소)을 함께 제거한다; ② 사적 경로의 노출창 [τ*, H]는 계약 tenor(2030–2045) **밖**이다 — 현재 조건의 계약은 문제가 되는 연도에 도달하지 못한다 (감사 2026-08-04). 금융비용 채널(σ_B→스프레드→WACC→τ*, S2)을 배선해도 이 지연은 남는다: 그 채널은 이 추정기의 MC 해상도 아래다. **조합(package)만이 τ*와 gap을 동시에 움직인다** ({{iv.POSCO.package_dtau}}y, {{iv.POSCO.package_dgap}} MtCO₂). carbon reform은 gap을 닫으면서 가격되는 탄소 부담을 **키운다** — timing 수단이지 risk 절감 수단이 아니다.
