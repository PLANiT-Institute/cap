...
## R1 {#referee-1}
λ 균일성(A5)은 P1의 실질 전제다. Prop 1 자체는 동차성에 의한 항등식이라 trivial 비판 가능. → **대응**: s05 λ_k 감응도 모듈 (`outputs/lambda_k_sensitivity.json`, 최대 share 이동 6.7%), A5의 공리 승격(완료).
...
## R5 {#referee-5}
p_bind가 행사정책(τ*)에 미도입 — LSM은 예산 없는 measure에서 풀고 p_bind는 밖에서 곱함. 내적 비일관. → **대응**: s03 실험 플래그 `p_bind_in_exercise` (기본 off, 현행 OFF). intervention 엔진은 τ*를 파라미터 변환 후 재계산하므로 '수준 따로 타이밍 따로' 비판의 절반은 해소. `status: OPEN`

## R6 {#referee-6}
수익 측면 부재 — 그린스틸 프리미엄, CBAM 수출가격 채널. anatomy는 **gross exposure**임을 명시. `status: SCOPE-NOTE`

## R7 {#referee-7}
**driver 정의의 자유도** — P1은 scalar λ·p_bind에 대한 불변성만 말한다. share를 실제로 움직이는 더 큰 자유도는 **driver를 어떻게 정의·분리하는가**이며, CAP의 다섯 driver는 강하게 상관돼 있다(ρ(h2,elec)=0.7). 현재 이 자유도는 `config/routes.csv`의 감응도 벡터 구성에 암묵적으로 고정돼 있고 대안 분해와 비교된 적이 없다. [[roncalliweisang2016]]은 팩터 단위 Euler 분해에서 **중복 팩터 시스템의 기여도가 유일하지 않음**을 보인다 — CAP은 driver 5개에 자산이 더 많아 그 조건에 직접 걸리지는 않으나, 팩터 기여도가 시스템 구성에 의존한다는 일반 논점은 그대로 적용된다.

**주의**: 최초 서술은 "Euler 분해가 팩터 회전에 불변이 아니다"였으나 원문 대조 결과 그 논문은 회전 비불변성을 증명하지 않는다(`PAPER_DIFF.md` 갱신 9). 회전 비불변성 자체를 주장하려면 별도 출처가 필요하다. → **미대응**. `status: OPEN — 문헌조사에서 신설(2026-07), 인용 정정됨`
